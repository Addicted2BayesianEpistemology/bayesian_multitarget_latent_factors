...
model {
  ...
  // lklhood
  to_vector(y1 - B1*theta1)[y1_missing] ~ normal(0, psi1*psi_noise_scale_multiplier_1);
//  target += normal_lupdf(to_vector(y1_missing).*to_vector(y1 - B1*theta1)/(psi1*psi_noise_scale_multiplier_1) | 0, 1); // the idea is the zeros here appear as constant additive terms
//  target += -(N*L1 - N_missing_y1)*log(psi1*psi_noise_scale_multiplier_1);

  to_vector(y2 - B2*theta2)[y2_missing] ~ normal(0, psi2*psi_noise_scale_multiplier_2);
//  target += normal_lupdf(to_vector(y2_missing).*to_vector(y2 - B2*theta2)/(psi2*psi_noise_scale_multiplier_2) | 0, 1);
//  target += -(N*L2 - N_missing_y2)*log(psi2*psi_noise_scale_multiplier_2);
...
generated quantities {
  ...
  matrix[L2,N] y2_predictive;
  {
    ...
    for (i in 1:N) {
      ...
      // With the slicing approach the exact computation of the likelihood is impervious, which is why we approach this using a Monte Carlo approach

      log_lik_y[i] = 0;
      /*
      for (s in 1:16) {

        vector[k] eta_internal = to_vector( normal_rng(beta*X[:,i], 1) );

        vector[p1] theta1_internal = to_vector( normal_rng(Lambda1*eta_internal, theta_noise_scale_multiplier_1) )./sqrt(tau_theta1);
        vector[p2] theta2_internal = to_vector( normal_rng(Lambda2*eta_internal, theta_noise_scale_multiplier_2) )./sqrt(tau_theta2);

        real target_aux =  normal_lpdf(y1_missing[:,i].*(y1[:,i] - B1*theta1_internal)/(psi1*psi_noise_scale_multiplier_1) | 0, 1);
        target_aux += -sum(y1_missing[:,i])*log(psi1*psi_noise_scale_multiplier_1);
        target_aux += N_missing_y1*0.5*log(2*pi());
        target_aux += normal_lpdf(y2_missing[:,i].*(y2[:,i] - B2*theta2_internal)/(psi2*psi_noise_scale_multiplier_2) | 0, 1);
        target_aux += -sum(y2_missing[:,i])*log(psi2*psi_noise_scale_multiplier_2);
        target_aux += N_missing_y2*0.5*log(2*pi());

        log_lik_y[i] = log_sum_exp(log_lik_y[i], target_aux);
      }
