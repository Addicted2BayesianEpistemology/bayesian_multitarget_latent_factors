...
data {
  ...
  // transformed data trick
  matrix[p1,N] proj_y1;     // B_1^+ y1
  matrix[p2,N] proj_y2;     // B_2^+ y2
  vector[N] ress_y1;        // sqrt(Y'M'MY)
  vector[N] ress_y2;        // sqrt(Y'M'MY)
  matrix[p1,p1] qr_decompose_R1;
  matrix[p2,p2] qr_decompose_R2;
...
//  to_vector(y2 - B2*theta2) ~ normal(0, psi2*psi_noise_scale_multiplier_2);
  {
    /*
    array[N] vector[p1] aux_theta1;
    array[N] vector[p2] aux_theta2;
    for (i in 1:N) {
      aux_theta1[i] = theta1[:,i];
      aux_theta2[i] = theta2[:,i];
    }
    */

    target += normal_lupdf(ress_y1./(psi1*psi_noise_scale_multiplier_1) | 0, 1);
    target += normal_lupdf(ress_y2./(psi2*psi_noise_scale_multiplier_2) | 0, 1);

    target += normal_lupdf( to_vector( qr_decompose_R1*(proj_y1 - theta1) )./(psi1*psi_noise_scale_multiplier_1) | 0, 1 );
    target += normal_lupdf( to_vector( qr_decompose_R2*(proj_y2 - theta2) )./(psi2*psi_noise_scale_multiplier_2) | 0, 1 );

    target += -N*L1*log(psi1);
    target += -N*L2*log(psi2);

//    proj_y1 ~ multi_normal_prec(aux_theta1, precision_B1/square(psi1*psi_noise_scale_multiplier_1));
//    target += -(L1-p1-1)*log(psi1*psi_noise_scale_multiplier_1);
//    ress_y2 ~ normal(0, psi2*psi_noise_scale_multiplier_2);
//    proj_y2 ~ multi_normal_prec(aux_theta2, precision_B2/square(psi2*psi_noise_scale_multiplier_2));
//    target += -(L2-p2-1)*log(psi2*psi_noise_scale_multiplier_2);
  }
